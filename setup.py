...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_robots_txt", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_robots_txt": ["*.pyi", "py.typed"],
        "tree_sitter_robots_txt.queries": ["*.scm"],
    },
    ext_package="tree_sitter_robots_txt",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_robots_txt/binding.c",
                "src/parser.c",
            ],
            define_macros=[
